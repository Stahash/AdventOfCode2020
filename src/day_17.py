...
def change_the_cube_4dim(cube):
    ...
    for i in range(1, cube_size-1):
        for j in range(1, cube_size-1):
            for k in range(1, number_layers-1):
                for m in range(1, number_layers - 1):

                    little_cube = zeros((3, 3, 3, 3), dtype=str, order='C')
                    little_cube.fill('.')
                    little_cube = cube[i-1:i+2, j-1:j+2, k-1:k+2, m-1:m+2]
                    active_neighbours = count_nonzero(little_cube == '#')

                    # limits increased by 1, because little cube is copy of the cube part and it's center can be '#'
                    if cube[i, j, k, m] == '#' and 3 <= active_neighbours <= 4:
                        modified_cube[i, j, k, m] = '#'

                    if cube[i, j, k, m] == '.' and active_neighbours == 3:
                        modified_cube[i, j, k, m] = '#'
...
def second_part(data, cycles):

    print('Second part:')
    cube_size = len(data[0]) + 4
    number_layers = 5
    cube = zeros((cube_size, cube_size, number_layers, number_layers), dtype=str, order='C')
    cube.fill('.')

    # initialize the cube
    for line_i in range(2, cube_size - 2):
        cube[line_i, 2:cube_size - 2, 2, 2] = list(data[line_i - 2])
    active_cubes = count_nonzero(cube == '#')

    for cycle_i in range(cycles):
        new_cube = change_the_cube_4dim(cube)
        ...
        cube = zeros((cube_size, cube_size, number_layers, number_layers), dtype=str, order='C')
        cube.fill('.')
        cube[2:cube_size - 2, 2:cube_size - 2, 2:number_layers - 2, 2:number_layers - 2] = new_cube
...
def day_17_solution(folder_name, file_name):
    ...
    answer_2 = second_part(input_data, cycles_number)
